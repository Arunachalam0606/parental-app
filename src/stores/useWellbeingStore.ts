...
interface WellbeingActions {
  ...
  reset: () => void
  setWellbeingSubPage: (
    subPage: "home" | "dashboard" | "goal" | "report" | "set-timer"
  ) => void
...
export const useWellbeingStore = create<WellbeingState & WellbeingActions>(
  (set) => ({
    ...initialState,
      ...
      set((state) => ({
        appStats: state.appStats.map((app) =>
          app.id === appId
            ? {
                ...app,
                limitMinutes: limitMinutes === 0 ? undefined : limitMinutes,
              }
            : app
        ),
      })),
    ...
    togglePersonalManualLock: (appId) =>
      set((state) => ({
        appStats: state.appStats.map((app) =>
          app.id === appId
            ? { ...app, isLockedManually: !app.isLockedManually }
            : app
        ),
      })),
      ...
      set((state) => ({
        childProfiles: state.childProfiles.map((p) => {
          ...
          return {
            ...p,
            weekdayLimitMinutes:
              type === "weekday" ? limitMinutes : p.weekdayLimitMinutes,
            weekendLimitMinutes:
              type === "weekend" ? limitMinutes : p.weekendLimitMinutes,
          }
        }),
      })),
    ...
    handleExtraTimeRequest: (requestId, action) =>
      set((state) => {
        ...
        const childName =
          state.childProfiles.find((c) => c.id === request.childId)?.name ||
          "Child"
        const appName =
          state.appStats.find((a) => a.id === request.appId)?.name || "App"
        const toastMessage =
          action === "approve"
    ...
    simulateActivityTick: () =>
      set((state) => {
        ...
        const updatedCategories = state.categories.map((cat) => {
          const matchingApps = updatedAppStats.filter(
            (a) => a.category === cat.name
          )
          const totalTime = matchingApps.reduce(
            (acc, a) => acc + a.timeSpent,
            0
          )
        ...
        const blockRate =
          (state.blockerToggles.blockAds ? 2 : 0) +
          (state.blockerToggles.blockTrackers ? 1 : 0)
        ...
        if (shouldBlock && blockRate > 0) {
          ...
          const randomType: "ad" | "tracker" | "social" | "annoyance" =
            randomDomain.includes("ads")
              ? "ad"
              : randomDomain.includes("tracking")
                ? "tracker"
                : "social"
          ...
          updatedStats = {
            adsBlocked:
              state.blockerStats.adsBlocked + (randomType === "ad" ? 1 : 0),
            trackersBlocked:
              state.blockerStats.trackersBlocked +
              (randomType === "tracker" ? 1 : 0),
            dataSavedMb: parseFloat(
              (state.blockerStats.dataSavedMb + Math.random() * 0.35).toFixed(2)
            ),
          }
    ...
    setDemoEmpty: (val) => set({ demoEmpty: val }),
  })
)
